...
if has('cscope')
  ...
  " If cscope.out exists, use it... it can be generated with 'cscope -q -R -b'
...
" ---- Mappings ----
" ------------------
" use C-] over a word go to function definition over the cursor
" " use C-\ over a word go to symbol over the cursor
if has('cscope')
...
function! SignOff()
  call append(line('.'), 'Signed-off-by: hgiraldos <hgiraldos@gmail.com>')
endfunction
